...
出力形式: JSON Schema Structured Output を要求し、形式不良時は本文専用の有限再整形を最大1回実施。workflow のアウトラインは壊れたJSONに限り再整形を最大2回実施。

API キー、文体カード本文、セッションの初期入力は記録しない。生成本文は Issue #12 の評価入力に対する出力であり、内容確認用に結果フォルダーへ保存する。

## ベースライン

変更前の実生成結果は取得できなかった。`baseline/` は空のままであり、スタブやプロンプトテストを変更前結果の代替にはしない。
...
文字数、見出し数、項目数は合否に使っていない。形式修復は内容品質の合格判定とは分離し、本文を回収できた後に keep/drop を確認した。

## 未検証・人手で必要な作業

- 変更前と変更後の同一モデル比較は、変更前ベースラインがないため未検証。
