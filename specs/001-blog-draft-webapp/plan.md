...
**Performance Goals**: 単一リクエストで RAG 検索 + LLM 生成を 600 秒以内に完了。同時接続 10 ユーザーまで、レスポンス時間の増加を 20% 以内に抑える。  
**Constraints**:  
- タイムアウト: デフォルト 600 秒（設定可能）。0=無制限は採用しない。  
...
| タイムアウト | 固定 600 秒（自動リトライなし） | Web 制約、コスト抑制、ユーザー判断を優先 |
