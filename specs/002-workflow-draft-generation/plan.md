...
## Canonical Scope (Authoritative)

- ワークフローの状態遷移は `Step1_Outline -> Step2_Draft -> Completed`。
- 下書き生成時の入力は「初期入力 + 固定RAGスナップショット + 確定アウトライン」。
- タイトル/冒頭段落案生成は WorkflowOrchestrator の外側にある独立エンドポイントで処理する。
- `/workflow/title` 系の旧導線は互換リダイレクト扱いで、利用導線は `/title-hook` を正とする。
