...
`StyleCard:SystemPrompt` と `StyleCard:Content` には、入力ポイントを記事の中心に置き、その理解に必要な情報だけを限定的に補完する方針を設定してください。テーマ全体の概要、入力にない機能やユースケース、一般的な全手順などを「記事として完全に見せる」目的だけで追加しないようにします。文体カードの実際の内容は秘匿情報として扱い、リポジトリへ保存しないでください。
