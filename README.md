...
入力ポイントの中心性と限定的な補完範囲は、アプリケーションが共通編集ポリシーとして System Message に追加します。`StyleCard:SystemPrompt` と `StyleCard:Content` は、語調、段落、構成、情報密度などの表現規則を設定するために使用してください。文体カードの簡潔さや情報量を抑える方針は維持され、必要な補完まで一律に禁止する指示と競合する場合に限り、共通編集ポリシーが優先されます。文体カードの実際の内容は秘匿情報として扱い、リポジトリへ保存しないでください。
