...
public sealed class LlmErrorClassifier
{
    public LlmErrorInfo Classify(Exception exception)
    {
        if (exception is LlmException llmException)
        {
            ...
            if (string.Equals(llmException.ErrorCode, "LLM_INVALID_RESPONSE", StringComparison.Ordinal))
            {
                return new LlmErrorInfo("LLM_INVALID_RESPONSE", "LLMから有効な編集計画を取得できませんでした", llmException.IsRetryable, 502);
            }
